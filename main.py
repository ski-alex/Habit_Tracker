...
if not Habit.check_habits_exist(habits):
    print ("Here is a quick overview of your currently tracked habits (active and broken):")
...
def cli_sub1():
    ...
    while True:
        ...
        if choice == "Filter habits":
            ...
            print(f"\nHere are all habits which can be deleted:")
            display.display_habits(habits, status_request = None,  length = "full", filter_period = [1, 2, 7])
            Habit.delete(habits)
            habits_store.save(habits)
        elif choice == "Duplicate a habit":
            print(f"\nHere are all habits which can be duplicated:")
            display.display_habits(habits, status_request = None,  length = "full", filter_period = [1, 2, 7])
            Habit.duplicate(habits)
            habits_store.save(habits)
        elif choice == "Adjust a habit":
            print(f"\nHere are all habits which can be adjusted:")
            display.display_habits(habits, status_request = "Established",  length = "full", filter_period = [1, 2, 7])
            Habit.adjust(habits)
            habits_store.save(habits)
...
def cli_sub2():
    ...
    Displays options to analyse the habits with predefined analysefunctions, or return to the main menu. 
    """
    while True:
        print(f"\n \\\ SUB MENU - ANALYSE //")
        choice = questionary.select(
            ...
            choices=["All currently tracked habits", "All habits with the same periodicity", "Longest run streak of all defined habits", 
                     "Longest run streak for a given habit","Longest active streaks", "Most interruptions since creation (Top 3)", 
                     "Most checks since creation (Top 3)", "Longest expired (Top 3)","Group by category","Go back to Main Menu"]
            ).ask()
        ...
        elif choice == "All currently tracked habits":
            print(f"\nHere is an overview of all currently tracked habits (not established now):")
            display.display_habits(habits, status_request = "Established", length = "short", filter_period = [1, 2, 7])
        else: # "back" was chosen
            print("Back to Main Menu")
            break
